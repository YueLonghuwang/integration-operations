...
public class BackEndFrameworkApplicationInit implements ApplicationRunner {
	...
	@Autowired
	public BackEndFrameworkApplicationInit(TCPThread tcpThread, RoleService roleService, UserService userService,
										   HostRepository hostRepository, WebSendToCService webSendToCService,
										   CMDSerialNumberRepository cmdSerialNumberRepository, JavaClientUtil javaClientUtil) {
...
//        this.javaClientUtil = javaClientUtil;
	...
	public void run(ApplicationArguments args) {
		...
		if (!roleEntityList.isEmpty()) {
			...
			System.out.println("系统成功初始化" + roleEntityList.size() + "个角色");
		}
		// 建立系统用户
		...
		if (!userEntityList.isEmpty()) {
			...
			System.out.println("系统成功初始化" + userEntityList.size() + "个用户");
		}
		// 初始化host
